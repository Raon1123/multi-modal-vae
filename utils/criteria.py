...
def iwae_vanilla(model, x, K):
    ...
    lw_list = []
    for _x in x.split(S):
        x_hat, zs, mu, logvar = model(_x, K=K, c=None)
...
def iwae_c(model, data, K):
    return iwae_vanilla(model, data, K)

# aux_objective: auxiliary objective loss type. 'cls' for classification, 'entropy' for Shannon entropy
def cvae_objective_wrapper(model, data, K=1, aux_objective='cls_min', classifier=None):
    x, c = data
    # elbo
    x_hat, z, mu, logvar = model(x, c, K)
    x_expand = x.expand(x_hat.size(0), *x.size())
    bce_loss = F.binary_cross_entropy(x_hat, x_expand, reduction='sum') * model.scaling_factor
    kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    ...
    if aux_objective != '' and classifier is not None:
        samples, c_gen = model.generate(run_path='', epoch=-1, N=K, K=1, c=None, vis=False)
        c_gen = c_gen.to(samples.device)
        ...
        if aux_objective=='cls_min': # cross entropy minimization
            ...
            aux_loss = aux_loss + F.cross_entropy(logit, c_gen, reduction='sum')
    ...
    total_loss = (bce_loss + kld_loss + aux_loss) / x.size(0)
    return total_loss
